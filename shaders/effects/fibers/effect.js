...
class FibersEffect {
  ...
  invalidateResources() {
    ...
    if (resources.resourceSet?.destroyAll) {
      try {
        ...
        logWarn?.('Failed to destroy fibers resources during invalidation:', error);
      }
    }
    ...
    if (resources.outputTexture?.destroy) {
      try {
        ...
        logWarn?.('Failed to destroy fibers output texture during invalidation:', error);
      }
    }
  ...
  async ensureResources({ device, width, height, multiresResources }) {
    if (!device) {
      throw new Error('FibersEffect.ensureResources requires a GPUDevice.');
    }
    if (!multiresResources?.outputTexture) {
      throw new Error('FibersEffect.ensureResources requires multires output texture.');
    }
  ...
  async updateParams(updates = {}) {
    ...
    Object.keys(updates).forEach((name) => {
      if (!PARAMETER_DEFINITIONS.has(name)) {
        logWarn?.(`updateFibersParams: Unknown parameter '${name}'.`);
      }
    });
    ...
    if (hasOwn(updates, 'speed')) {
      ...
      if (numeric === null) {
        logWarn?.('updateFibersParams: speed must be a finite number.');
      } else {
  ...
  #getParameterDefinition(name) {
    ...
    if (!definition) {
      throw new Error(`Fibers metadata missing parameter definition for '${name}'.`);
    }
    return definition;
  }
  ...
  #getNumericBounds(name) {
    ...
    if (!Number.isFinite(min) || !Number.isFinite(max)) {
      throw new Error(`Fibers parameter '${name}' must define finite min and max bounds.`);
    }
    if (min > max) {
      throw new Error(`Fibers parameter '${name}' metadata has min greater than max.`);
    }
    return { min, max };
  }
  ...
  #getBindingOffset(name) {
    ...
    if (!binding) {
      throw new Error(`Fibers metadata missing parameter binding for '${name}'.`);
    }
    if (binding.buffer !== 'params') {
      throw new Error(`Fibers parameter '${name}' binding must target the 'params' buffer.`);
    }
  ...
  #getParamsFloatLength() {
    ...
    if (!Number.isFinite(sizeBytes) || sizeBytes <= 0) {
      throw new Error('Fibers metadata must provide a positive params buffer size.');
    }
    if (sizeBytes % Float32Array.BYTES_PER_ELEMENT !== 0) {
      throw new Error('Fibers params buffer size must be divisible by 4 bytes.');
    }
  ...
  async #createResources({ device, width, height, multiresResources }) {
    ...
    setStatus?.('Creating fibers resources…');

    try {
      logInfo?.('Attempting to load fibers shader descriptor...');
      const descriptor = getShaderDescriptor('fibers');
      logInfo?.(`Fibers descriptor loaded: ${descriptor.label}`);
      const shaderMetadata = await getShaderMetadataCached('fibers');
      logInfo?.(`Fibers metadata parsed, bindings count: ${shaderMetadata.bindings.length}`);
      warnOnNonContiguousBindings?.(shaderMetadata.bindings, descriptor.id);

      logInfo?.(`Creating fibers resource set with input texture: ${Boolean(multiresResources.outputTexture)}`);
      ...
      logInfo?.(`Fibers resource set created. Buffers: ${Object.keys(resourceSet.buffers).join(', ')}, Textures: ${Object.keys(resourceSet.textures).join(', ')}`);
      ...
      Object.entries(bindingOffsets).forEach(([name, offset]) => {
        if (offset < 0 || offset >= paramsLength) {
          throw new Error(`Fibers parameter '${name}' binding offset ${offset} exceeds params buffer length ${paramsLength}.`);
        }
      });
      ...
      this.retryDelayMs = RETRY_BASE_DELAY_MS;
      setStatus?.('Fibers resources ready.');
      return this.resources;
    } catch (error) {
      logWarn?.('Failed to create fibers resources:', error);
  ...
  #createDisabledResources({ width, height }, reason) {
    ...
    if (reason) {
      logWarn?.(`Fibers effect disabled: ${reason}`);
    } else {
      logWarn?.('Fibers effect disabled.');
    }

    setStatus?.('Fibers effect unavailable. Using generator output directly.');
