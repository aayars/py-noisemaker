...
const PARAMETER_DEFAULTS = Object.freeze({
  horizontal: getDefaultParamValue('horizontal', false),
  displacement: getDefaultParamValue('displacement', 0.5),
  speed: getDefaultParamValue('speed', 1.0),
  enabled: getDefaultParamValue('enabled', true),
});

class FalseColorEffect {
  static id = meta.id;
  ...
  constructor({ helpers } = {}) {
    ...
    this.userState = {
      horizontal: Boolean(PARAMETER_DEFAULTS.horizontal),
  ...
  invalidateResources() {
    ...
    if (resources.resourceSet?.destroyAll) {
      try {
        ...
        logWarn?.('Failed to destroy false_color resources during invalidation:', error);
      }
    }
    ...
    if (resources.outputTexture?.destroy) {
      try {
        ...
        logWarn?.('Failed to destroy false_color output texture during invalidation:', error);
      }
    }
  ...
  async ensureResources({ device, width, height, multiresResources }) {
    if (!device) {
      throw new Error('FalseColorEffect.ensureResources requires a GPUDevice.');
    }
    if (!multiresResources?.outputTexture) {
      throw new Error('FalseColorEffect.ensureResources requires multires output texture.');
    }
  ...
  async updateParams(updates = {}) {
    if (!updates || typeof updates !== 'object') {
      throw new TypeError('FalseColorEffect.updateParams expects an object.');
    }
    ...
    Object.keys(updates).forEach((name) => {
      if (!PARAMETER_DEFINITIONS.has(name)) {
        logWarn?.(`updateFalseColorParams: Unknown parameter '${name}'.`);
      }
    });

    if (hasOwn(updates, 'horizontal')) {
      this.#getParameterDefinition('horizontal');
      const horizontal = Boolean(updates.horizontal);
      this.userState.horizontal = horizontal;
      if (this.resources?.paramsState) {
        const offset = this.resources?.bindingOffsets?.horizontal ?? this.#getBindingOffset('horizontal');
        this.resources.paramsState[offset] = horizontal ? 1.0 : 0.0;
        this.resources.paramsDirty = true;
      }
      updated.push('horizontal');
    }

    if (hasOwn(updates, 'displacement')) {
      const numeric = toFiniteNumber(updates.displacement);
      if (numeric === null) {
        logWarn?.('updateFalseColorParams: displacement must be a finite number.');
      } else {
    ...
    if (hasOwn(updates, 'speed')) {
      ...
      if (numeric === null) {
        logWarn?.('updateFalseColorParams: speed must be a finite number.');
      } else {
  ...
  getUIState() {
    return {
      horizontal: this.userState.horizontal,
  ...
  #getParameterDefinition(name) {
    ...
    if (!definition) {
      throw new Error(`FalseColor metadata missing parameter definition for '${name}'.`);
    }
    return definition;
  }
  ...
  #getNumericBounds(name) {
    ...
    if (!Number.isFinite(min) || !Number.isFinite(max)) {
      throw new Error(`FalseColor parameter '${name}' must define finite min and max bounds.`);
    }
    if (min > max) {
      throw new Error(`FalseColor parameter '${name}' metadata has min greater than max.`);
    }
    return { min, max };
  }
  ...
  #getBindingOffset(name) {
    ...
    if (!binding) {
      throw new Error(`FalseColor metadata missing parameter binding for '${name}'.`);
    }
    if (binding.buffer !== 'params') {
      throw new Error(`FalseColor parameter '${name}' binding must target the 'params' buffer.`);
    }
    const offset = Number(binding.offset);
    if (!Number.isFinite(offset)) {
      throw new Error(`FalseColor parameter '${name}' binding offset must be a finite number.`);
    }
    return offset;
  }
  ...
  #getParamsFloatLength() {
    ...
    if (!Number.isFinite(sizeBytes) || sizeBytes <= 0) {
      throw new Error('FalseColor metadata must provide a positive params buffer size.');
    }
    if (sizeBytes % Float32Array.BYTES_PER_ELEMENT !== 0) {
      throw new Error('FalseColor params buffer size must be divisible by 4 bytes.');
    }
  ...
  async #createResources({ device, width, height, multiresResources }) {
    ...
    setStatus?.('Creating false_color resources…');

    try {
      logInfo?.('Attempting to load false_color shader descriptor...');
      const descriptor = getShaderDescriptor('false_color');
      logInfo?.(`FalseColor descriptor loaded: ${descriptor.label}`);
      const shaderMetadata = await getShaderMetadataCached('false_color');
      logInfo?.(`FalseColor metadata parsed, bindings count: ${shaderMetadata.bindings.length}`);
      warnOnNonContiguousBindings?.(shaderMetadata.bindings, descriptor.id);

      logInfo?.(`Creating false_color resource set with input texture: ${Boolean(multiresResources.outputTexture)}`);
      ...
      logInfo?.(`FalseColor resource set created. Buffers: ${Object.keys(resourceSet.buffers).join(', ')}, Textures: ${Object.keys(resourceSet.textures).join(', ')}`);
      ...
      const bindingOffsets = {
        ...
        horizontal: this.#getBindingOffset('horizontal'),
      ...
      Object.entries(bindingOffsets).forEach(([name, offset]) => {
        if (offset < 0 || offset >= paramsLength) {
          throw new Error(`FalseColor parameter '${name}' binding offset ${offset} exceeds params buffer length ${paramsLength}.`);
        }
      });
      ...
      paramsState[bindingOffsets.horizontal] = this.userState.horizontal ? 1.0 : 0.0;
      ...
      this.retryDelayMs = RETRY_BASE_DELAY_MS;
      setStatus?.('FalseColor resources ready.');
      return this.resources;
    } catch (error) {
      logWarn?.('Failed to create false_color resources:', error);
  ...
  #createDisabledResources({ width, height }, reason) {
    ...
    if (reason) {
      logWarn?.(`FalseColor effect disabled: ${reason}`);
    } else {
      logWarn?.('FalseColor effect disabled.');
    }

    setStatus?.('FalseColor effect unavailable. Using generator output directly.');
...
export default FalseColorEffect;
