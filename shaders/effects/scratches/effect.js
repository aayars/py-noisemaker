...
const WORM_STEPS_PER_FRAME = 20;  // Python uses duration 2-4, at sqrt(min(w,h)) per duration unit
...
// Worm behavior defaults (matching fibers for proper scaling)
const DEFAULT_DENSITY = 0.0525;
const DEFAULT_STRIDE = 0.125;
const DEFAULT_STRIDE_DEVIATION = 0.125;
const DEFAULT_INTENSITY = 1.0;  // Above threshold - trails stay solid forever
const DEFAULT_LIFETIME = 0.0;  // 0 = agents never respawn (static scratches)
...
function getBindingOffset(name) {
    ...
    if (!Number.isFinite(offset)) {
        throw new Error(`Scratches parameter '${name}' binding offset must be a finite number.`);
    }
    return offset;
}
...
function seedAgents(width, height, agentCount, strideConfig, rng) {
    ...
    for (let i = 0; i < agentCount; i += 1) {
        ...
        // Agent luminance - this gets multiplied by the sampled noise value
...
class ScratchesEffect {
    ...
    constructor({ helpers } = {}) {
        ...
        this.scratchesGenerated = false;  // Track if we've already generated static scratches
        this.scratchDuration = 0;  // Number of worm steps to simulate
    }

    async ensureResources({ device, width, height, multiresResources }) {
        if (!device) {
            throw new Error('ScratchesEffect requires a GPUDevice.');
        }
        if (!multiresResources?.outputTexture) {
            throw new Error('ScratchesEffect requires multires output texture.');
        }
    ...
    async updateParams(updates = {}) {
        if (!updates || typeof updates !== 'object') {
            throw new TypeError('ScratchesEffect.updateParams expects an object.');
        }
    ...
    beforeDispatch({ device }) {
        ...
        // For scratches, we generate a static cluster once
        if (this.scratchesGenerated) {
            return;  // Already generated, keep the static result
        }
        ...
        if (this.noiseSeed === null || this.noiseSeed !== currentSeed) {
            ...
            this.scratchesGenerated = false;  // Reset flag when seed changes
        }

        // Don't advance time - we want static scratches
        // Set time to 0 for initial generation
        if (this.wormParamsState) {
    ...
    invalidateResources() {
        ...
        try {
            ...
            this.helpers.logWarn?.('Failed to destroy scratches resource set:', error);
        }

        try {
            this.resources.outputTexture?.destroy?.();
        } catch (error) {
            this.helpers.logWarn?.('Failed to destroy scratches output texture:', error);
        }
    ...
    #createInitialWormParams(width, height, seed) {
        ...
        // Use similar low values to fibers, otherwise worms cover the entire image!
        const density = DEFAULT_DENSITY * (0.8 + rng() * 0.4);  // ~0.0525 with slight variation
        const stride = DEFAULT_STRIDE * (0.8 + rng() * 0.4);    // ~0.125 with slight variation
        const behavior = rng() < 0.5 ? 1 : 3;  // Obedient or unruly (not chaotic like fibers)
        const kink = 0.5 + rng() * 0.5;  // 0.5 to 1.0
...
export default ScratchesEffect;
