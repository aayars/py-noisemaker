// Applies the stray hair overlay by blending the base image with a brightness
// map using a worm mask. The shader mirrors the CPU implementation by
// performing per-channel blending with independently scaled brightness and mask
// tensors while respecting differing channel counts.

struct StrayHairParams {
  size : vec4<f32>;      // width, height, baseChannels, brightnessChannels
  factors : vec4<f32>;   // maskChannels, brightnessScale, maskScale, unused
};
...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
  ...
  let brightnessScale = params.factors.y;
  let maskScale = params.factors.z;
  ...
  for (var channel : u32 = 0u; channel < baseChannels; channel = channel + 1u) {
    ...
    let brightValue = sampleChannel(brightnessSample, channel, brightnessChannels);
    let maskValue = sampleChannel(maskSample, channel, maskChannels);
    let scaledMask = clamp(maskValue * maskScale, 0.0, 1.0);
    let scaledBrightness = brightValue * brightnessScale;
    result[channel] = baseValue * (1.0 - scaledMask) + scaledBrightness * scaledMask;
  }
