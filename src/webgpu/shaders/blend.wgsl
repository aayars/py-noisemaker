...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
  ...
  let maskChannelCount : u32 = max(u32(params.maskChannels), 1u);
  let coords : vec2<i32> = vec2<i32>(i32(gid.x), i32(gid.y));
  let sampleA : vec4<f32> = textureLoad(inputA, coords, 0);
  let sampleB : vec4<f32> = textureLoad(inputB, coords, 0);
  let maskSample : vec4<f32> = textureLoad(maskTexture, coords, 0);
  ...
  loop {
    ...
    var maskChannel : u32 = channel;
    if (maskChannel >= maskChannelCount) {
      maskChannel = 0u;
    }
    let maskVal : f32 = clamp(readChannel(maskSample, maskChannel), 0.0, 1.0);
    let blended : f32 = mix(aVal, bVal, maskVal);
