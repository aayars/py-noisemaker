...
fn main(@builtin(global_invocation_id) gid : vec3<u32>) {
  ...
  let scaleX : f32 = quadDirectional ? baseScaleX : baseScaleX * 2.0;
  let scaleY : f32 = quadDirectional ? baseScaleY : baseScaleY * 2.0;

  var refX : f32 = textureLoad(referenceXTex, vec2<i32>(i32(gid.x), i32(gid.y)), 0).x;
  var refY : f32 = textureLoad(referenceYTex, vec2<i32>(i32(gid.x), i32(gid.y)), 0).x;
